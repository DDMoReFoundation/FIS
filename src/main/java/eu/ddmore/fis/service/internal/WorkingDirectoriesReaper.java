 ...
 * 
 * TODO This implementation won't work once FIS starts to manage directories managing job files.
 */
@Component
    ...
    public void performCleanup() {
    	...
    	File[] forDeletion = workingDirectory.listFiles(new FileFilter() {
    		...
    		public boolean accept(File pathname) {
    			boolean forDeletion = pathname.isDirectory() && (new Date().getTime() - pathname.lastModified() > taskWorkingDirectoryAvailabilityTimeout);
    			LOG.debug(String.format("Directory %s last modified at %s is %s scheduled for deletion.", pathname, pathname.lastModified(), (forDeletion?"":"not")));
    			return forDeletion;
    		}
    	});
    	...
    	LOG.debug(String.format("Deleted %s working directories.", deleted));
    }
